...
const TwictchStreamDetector = async (client, TWITCH_CLIENT_ID, TWITCH_CLIENT_SECRET) => {
      ...
      const run = async function Run() {
        streamers.forEach(async ({
          ...
          await twitch.getStreams({
            ...
            const {
              twitchOldState: oldState,
              twitchNewState: newState
            } = await (0, _subFunctions.UserData)(guild, {
              id: discordId
            }, {
              type: 'twitch',
            ...
            if (!oldState[0] && newState[0]) {
              ...
              const channel = await (0, _subFunctions.twitchLiveStreamTempChannels)(server, guild, server.TwitchApi.liveStreamCategoryId, true, name, discordId);
  ...
  join stream queue: <#${channel.id}>`,
              ...
              (0, _subFunctions.twitchLiveStreamTempChannels)(server, guild, server.TwitchApi.liveStreamCategoryId, false, name, discordId);
            }
          });
        });
      };
